...
public class PlayerActions : MonoBehaviour
{
    ...
    public BoxCollider firstTriggerCollider;
    ...
    public void Update()
    {
        ...
        //Handles Pause Menu
        if (pauseCheck == false && Keyboard.current.escapeKey.wasPressedThisFrame && pauseTimer > 0.1f && dialogueUI.IsOpen == false && cutscenePlaying == false && TimelinePlaying.timelinePlaying == false)
        {
            OpenPauseMenu();
        }
    ...
    {
        if (other.CompareTag("DialogueTrigger") && waitingForFirst == true)
        {
    ...
    void CrosshairChange(bool on)
    {
        ...
        else if (pauseCheck == false && ResponseHandler.isThereButtons == false && cutscenePlaying == false)
        {
            cursor.SetActive(true);
            redCursor.SetActive(false);
        }
